...
def lookup_tmc_uart(config, max_addr):
    ...
    uart_pin_params = ppins.lookup_pin(config.get('uart_pin'), can_pullup=True, share_type="tmc_uart")

    instance_id = 0
    mcu_uart = uart_pin_params.get('class')
    if mcu_uart is None:
        mcu_uart = MCU_TMC_uart_device(uart_pin_params)
        uart_pin_params['class'] = mcu_uart

    return instance_id, addr, mcu_uart

class MCU_TMC_uart_device:  
    def __init__(self, uart_pin_params):
        self.uart_pin = uart_pin_params['pin']
        self.mcu = uart_pin_params['chip']
        self.cmd_queue = self.mcu.alloc_command_queue()
        self.mutex = lookup_tmc_uart_mutex(self.mcu)
        self.mcu.register_config_callback(self.build_config)
    ...
    def build_config(self):
        self.mcu.add_config_cmd(
            "config_tmcuart_device uart_pin=%s baud=%u"
            % (self.uart_pin, TMC_BAUD_RATE))
        self.tmcuart_send_cmd = self.mcu.lookup_query_command(
            "tmcuart_device_send addr=%u reg=%u read=%c write=%u",
            "tmcuart_device_response success=%c read=%u", cq=self.cmd_queue, is_async=True)

    def reg_read(self, instance_id, addr, reg):
        params = self.tmcuart_send_cmd.send([addr, reg, 1, 0])
        if params['success'] == 0:
            return None
        return params['read']
    ...
    def reg_write(self, instance_id, addr, reg, val, print_time=None):
        ...
        if print_time is not None:
            minclock = self.mcu.print_time_to_clock(print_time)
        self.tmcuart_send_cmd.send([addr, reg, 0, val], minclock=minclock)
    ...
    def set_register(self, reg_name, val, print_time=None):
        ...
        with self.mutex:
            for retry in range(10):
                ...
                self.ifcnt = None
                logging.info("Wrong IFCNT %d Expected: %d", self.ifcnt, ifcnt + 1)
        raise self.printer.command_error(
